...
export function ClientActions({
  ...
  const [editUris, setEditUris] = useState(redirectUris.join('\n'));

  async function patch(body: object) {
    const res = await fetch(`/api/settings/oauth-clients/${clientId}`, {
      method: 'PATCH',
    ...
    if (!res.ok) {
      ...
      if (
        (data.error === 'INVALID_NAME' || data.error === 'INVALID_REDIRECT_URIS') &&
  ...
  async function saveEdits() {
    ...
    setError(null);
    try {
      await patch({ name: editName.trim(), redirectUris: uris });
      setEditing(false);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Network error');
  ...
  async function hardDelete() {
    const cascadeWarning =
      authCodeCount > 0 || pendingAuthCount > 0
        ? `\n\nThis will also remove ${authCodeCount} auth code${
            authCodeCount === 1 ? '' : 's'
          } and ${pendingAuthCount} pending auth${pendingAuthCount === 1 ? '' : 's'}.`
        : '';
    if (
      !confirm(
        `Permanently delete '${name}' (${clientId})?\n\nThis cannot be undone — the client_secret cannot be recovered. Use "Deactivate" instead if you might want to restore the client.${cascadeWarning}`,
      )
    ) {
      return;
    }
    ...
    return (
      <div className="space-y-2 min-w-[260px]">
        ...
        <div className="flex items-center gap-2 justify-end">
          <button
            onClick={saveEdits}
            disabled={busy !== null || !editName.trim() || !editUris.trim()}
